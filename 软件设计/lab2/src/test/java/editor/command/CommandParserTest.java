...
public class CommandParserTest {
    ...
    @Test
    void testParseInitTextType() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("init text test.txt");
        assertNotNull(cmd);
        assertEquals("init", cmd.getCommandName());
        assertEquals("text", cmd.getArg(0));
        assertEquals("test.txt", cmd.getArg(1));
    }

    @Test
    void testParseInitXmlWithLog() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("init xml config.xml with-log");
        assertNotNull(cmd);
        assertEquals("init", cmd.getCommandName());
        assertEquals("xml", cmd.getArg(0));
        assertEquals("config.xml", cmd.getArg(1));
        assertEquals("with-log", cmd.getArg(2));
    }
    ...
    @Test
    void testParseInsertBefore() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("insert-before book newBook book1 \"\"");
        assertNotNull(cmd);
        assertEquals("insert-before", cmd.getCommandName());
        assertEquals("book", cmd.getArg(0));
        assertEquals("newBook", cmd.getArg(1));
        assertEquals("book1", cmd.getArg(2));
        assertEquals("", cmd.getArg(3));
    }
    ...
    void testParseAppendChild() {
        ...
        assertNotNull(cmd);
        assertEquals("append-child", cmd.getCommandName());
        assertEquals("price", cmd.getArg(0));
        assertEquals("price4", cmd.getArg(1));
        assertEquals("book1", cmd.getArg(2));
        assertEquals("29.99", cmd.getArg(3));
    }

    @Test
    void testParseEditId() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("edit-id oldId newId");
        assertNotNull(cmd);
        assertEquals("edit-id", cmd.getCommandName());
        assertEquals("oldId", cmd.getArg(0));
        assertEquals("newId", cmd.getArg(1));
    }

    @Test
    void testParseEditText() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("edit-text title1 \"New Title\"");
        assertNotNull(cmd);
        assertEquals("edit-text", cmd.getCommandName());
        assertEquals("title1", cmd.getArg(0));
        assertEquals("New Title", cmd.getArg(1));
    }

    @Test
    void testParseDeleteElement() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("delete-element book1");
        assertNotNull(cmd);
        assertEquals("delete-element", cmd.getCommandName());
        assertEquals("book1", cmd.getArg(0));
    }

    @Test
    void testParseXmlTreeWithFile() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("xml-tree books.xml");
        assertNotNull(cmd);
        assertEquals("xml-tree", cmd.getCommandName());
        assertEquals("books.xml", cmd.getArg(0));
    }

    @Test
    void testParseXmlTreeWithoutFile() {
    ...
    @Test
    void testParseSpellCheckWithFile() {
        CommandParser.ParsedCommand cmd = CommandParser.parse("spell-check test.txt");
        assertNotNull(cmd);
        assertEquals("spell-check", cmd.getCommandName());
        assertEquals("test.txt", cmd.getArg(0));
    }

    @Test
    void testParseSpellCheckWithoutFile() {
